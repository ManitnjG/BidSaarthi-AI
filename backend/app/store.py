...
def upsert(items):
 with lock,sqlite3.connect(DB) as c:
  for t in items:c.execute("insert into tenders(id,source_id,source_url,title,closes_at,content_hash,payload) values(?,?,?,?,?,?,?) on conflict(id) do update set payload=excluded.payload,content_hash=excluded.content_hash,updated_at=current_timestamp",(t.id,t.source_id,t.source_url,t.title,t.closes_at,t.content_hash,t.model_dump_json()))
